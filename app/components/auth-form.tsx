...
const MODE_COPY: Record<AuthMode, { title: string; submit: string; switchLabel: string; switchHref: string; switchText: string }> =
  {
    'sign-in': {
      title: '登入帳戶',
      submit: '登入',
...
export default function AuthForm({ mode }: AuthFormProps) {
      ...
      startTransition(async () => {
        try {
          ...
          if (mode === 'sign-up') {
            ...
            setMessage('註冊成功！請前往 Email 完成驗證，或直接使用密碼登入。');
            setFields({ email, password: '', confirmPassword: '' });
          } else {
            const { error: signInError } = await supabase.auth.signInWithPassword({
              email,
  ...
  return (
    <form
      onSubmit={handleSubmit}
      className="w-full max-w-md space-y-6 rounded-2xl border border-slate-800 bg-slate-900/60 p-8 shadow-lg shadow-black/30 backdrop-blur"
    >
      <header className="space-y-2 text-center">
        <h1 className="text-2xl font-semibold tracking-tight text-white">{title}</h1>
        <p className="text-sm text-slate-400">使用 Email 與密碼完成 {mode === 'sign-up' ? '註冊' : '登入'}。</p>
      </header>
